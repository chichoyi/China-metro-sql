...
def output_lines():
    with open('D:\\clines.csv', 'a+') as f:
        print('城市行政区划代码', '城市名', '线路id', '线路名','sql', sep=',', file=f)
        for line_id, line in lines.items():
            print(line['cityName'], line['lineName'])
            print(line['cityCode'], line['cityName'],
                  line_id, line['lineName'],
                  'Insert into lines(city_id,city_name,line_id,line_name) value (`{}`,`{}`,`{}`,`{}`)'.format(
                      line['cityCode'], line['cityName'],line_id,line['lineName']
                  ), sep=',', file=f)

            sql = 'Insert into `lines`(city_id,city_name,line_id,line_name) values ("{}","{}","{}","{}")'.format(
                      line['cityCode'], line['cityName'],line_id,line['lineName']
                  )
            print(sql)
            # 执行sql语句
...
def connectDB():
    ...
    global db
    db = pymysql.connect(
        host="127.0.0.1",user="root",passwd="root",db="city_metro",port=3306,charset='utf8'
        )
    print('连接上了!')
    global cursor
    cursor = db.cursor()
    ...
    with open('stations.csv', 'a+') as f:
        for i in result['l']:
            for j in i['st']:
                ...
                print( station["stationPosition"])
                long =  str.split(station["stationPosition"],",")[0]
                la =  str.split(station["stationPosition"],",")[1]
                print(long,la)
                sql = 'insert into `stations` (city_id,city_name,station_id,`name`,`long`,la,line_id) ' \
                        ' values ("{}","{}","{}","{}","{}","{}","{}")'.format(
                            station["cityCode"],
                            station["cityName"],
                            station["stationId"],
